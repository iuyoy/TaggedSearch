...
#数据处理
class Get_wikdiata_entity_properties(object):
    ...
    def __init__(self):
        super(Get_wikdiata_entity_properties,self).__init__()
    #提取可能的标签,并且保存到数据库中
                ...
                if(key == 'type' or key == 'id'):
                    ...
                    for sub_key in self.keys[key]:
                        if (type(wiki_dict[key]) == dict and wiki_dict[key].has_key(sub_key)):
                            self.analyse_format[key+'_'+sub_key] = wiki_dict[key][sub_key]['value'].decode('raw_unicode_escape')
    ...
    def reset_analyse_format(self):
        self.analyse_format = {'type':'','id':'','labels_zh-hans':'','labels_zh-cn':'','labels_zh':'','labels_en':'','descriptions_zh-hans':'','descriptions_zh-cn':'','descriptions_zh':'','aliases':[]\
        ,u'main_classification':[],'father_classification':[],'property':[],'belong_to':[]} 
